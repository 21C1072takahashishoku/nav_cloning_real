...
class deep_learning:
    ...
    # ===== 修正後：2026-05-11 train→test切替時の自動保存対応 =====
    def save(self, save_path, tag=None):
        # <model save>
        if tag is None:
            tag = time.strftime("%Y%m%d_%H:%M:%S")

        path = os.path.join(save_path, str(tag))
        os.makedirs(path, exist_ok=True)

        model_path = os.path.join(path, "model_gpu.pt")
