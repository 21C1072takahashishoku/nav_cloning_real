...
class nav_cloning_node:
    def __init__(self):
        ...
        # dataset_id が空なら start_time を使う（collect_teleop+lux.py と同じ挙動）
        ...
        self.dataset_base = roslib.packages.get_pkg_dir('nav_cloning') + '/data/' + str(self.dataset_id) + '/dataset/'
        ...
        if self.save_offline_dataset:
            rospy.loginfo(f"[nav_cloning_node] offline dataset save ON: {self.dataset_base} (stride={self.dataset_stride})")
    ...
    def lux_callback(self, msg):
        # collect_teleop+lux.py と同じ: Illuminance.illuminance を保存
    ...
    def callback_model_save(self, data):
        # Trigger service なので TriggerResponse を返す
    ...
    def _maybe_save_offline_sample(self, img, img_left, img_right, expert_action, distance, executed_action, policy_action, phase):
        """オフライン学習用に (画像3枚 + CSV 1行) を保存する。

        - 画像: float32 HWC (48,64,3) の .npy
        - CSV: episode, center, left, right, lux, distance, expert, executed, policy, phase
               learning_surprise.py は center/left/right だけ使うので追加列があってもOK。
        """
        ...
        # float32 で保存（learning_surprise.py は float32 を想定）
        ...
        with open(self.vel_csv_file, 'a', newline='') as csvfile:
            ...
            if not file_exists:
                writer.writerow([
                    ...
                    'pose_x', 'pose_y', 'distance',
                    'expert', 'executed', 'policy', 'phase',
                ])
    ...
    def loop(self):
        ...
        if self.learning:
            ...
            # policy_action は act_and_trains の戻り値を優先
            ...
            if self.mode == "change_dataset_balance":
                ...
                # nav_cloning_node_pytorch.py と同じ切り替えロジック
            ...
            # 「実行は (expert or policy) でも、教師ラベルは expert_action(nav_vel)」として保存
            if self.cmd_vel_mode == "policy":
                # policy走行時でも学習は上で実施済み
                cmd_action = policy_action
            elif self.cmd_vel_mode == "expert":
                cmd_action = expert_action
            else:
                # mixedは nav_cloning_node_pytorch.py と同じ target_action を実行
            ...
            distance = self.min_distance
            # Test must run with policy output only (no expert/mixed switching).
            cmd_action = policy_action
